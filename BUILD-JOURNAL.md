...
- **#48** — bookworm round 3 shipped (book now faces the bookworm, eyes look down-right + track), awaiting user "looks good".
- **#7** — OAuth: Supabase secret now in `.env.local`, restart confirmed no Google warning. User still needs to (a) ensure `http://localhost:54321/auth/v1/callback` is whitelisted in Google Cloud Console → Credentials → OAuth client → Authorized redirect URIs, and (b) try Google sign-in end-to-end. Issue stays OPEN until that round-trip succeeds.
